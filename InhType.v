...
Proof.
  rewrite /ext. do ?insub_case; try slia. move=> ?? <- [/= ??? /double ? <- ?]. 
...
Lemma ext_add_n  {x n} {f : 'I_n -> T} :
  ext (add f x) n = x.
Proof. 
  rewrite /ext. insub_case; try slia. move=> [/=] ? L _ /esym E.
...
Proof.
  rewrite /comp2=> H. case L: (a < n).
  { rewrite ext_add //; try slia.
    case L': (b < n). 
    { rewrite ext_add //. slia. }
    rewrite {2}/ext. do ?insub_case; try slia.
    by move/1$/(codom_rfield _). }
  rewrite {1}/ext. do ?insub_case; try slia.
