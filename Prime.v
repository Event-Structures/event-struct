...
Definition left_clo {T : Type} (r : rel T) y := fun x => r x y.
...
Definition finite_causes_axiom := forall e, finite (left_clo le e).
...
HB.mixin Record prime_of_event_struct E of evstruct E := 
{
    ...
    consist_confl   : forall e1 e2 e3 : E, (confl e1 e2) = true -> (le e2 e3) = true -> (confl e1 e3) = true; (***)
}.
...
Variables (E : prime.type).
Implicit Types (p : pred E) (e : E) (s : seq E).

Definition confl_free p := forall e1 e2, e1 \in p -> e2 \in p -> ~~ (e1 # e2).
Definition config p := (confl_free p * left_closed E p)%type.

Lemma config_left_clo e: config (left_clo le e).
Proof.
split=> [e1 e2|]; last by apply left_closed_left_clo. 
case confl: (e1 # e2)=> //= /sw/(consist_confl _ _ _ confl).
rewrite symm_confl=> confl' /(consist_confl _ _ _ confl').
by rewrite confl_irrefl.
Qed.
